...
public class CssPauseBefore extends CssProperty {
    ...
    public void addToStyle(ApplContext ac, CssStyle style) {
        if (((Css2Style) style).cssPauseBefore != null)
            style.addRedefinitionWarning(ac, this);
        ((Css2Style) style).cssPauseBefore = this;
    }
    ...
    public boolean equals(CssProperty property) {
        return (property instanceof CssPauseBefore &&
    ...
    public CssProperty getPropertyInStyle(CssStyle style, boolean resolve) {
        if (resolve) {
            return ((Css2Style) style).getPauseBefore();
        } else {
            return ((Css2Style) style).cssPauseBefore;
        }
    }
}
