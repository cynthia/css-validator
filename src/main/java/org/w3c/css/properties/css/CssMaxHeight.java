// $Id: CssMaxHeight.java,v 1.2 2012-10-11 13:33:59 ylafon Exp $
...
public class CssMaxHeight extends CssProperty {
    ...
    public void addToStyle(ApplContext ac, CssStyle style) {
        if (((Css2Style) style).cssMaxHeight != null)
            style.addRedefinitionWarning(ac, this);
        ((Css2Style) style).cssMaxHeight = this;
    }
    ...
    public CssProperty getPropertyInStyle(CssStyle style, boolean resolve) {
        if (resolve) {
            return ((Css2Style) style).getMaxHeight();
        } else {
            return ((Css2Style) style).cssMaxHeight;
        }
    }
}
