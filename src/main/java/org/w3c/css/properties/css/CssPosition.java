// $Id: CssPosition.java,v 1.1 2012-09-28 09:51:34 ylafon Exp $
...
public class CssPosition extends CssProperty {
    ...
    public void addToStyle(ApplContext ac, CssStyle style) {
        if (((Css2Style) style).cssPosition != null)
            style.addRedefinitionWarning(ac, this);
        ((Css2Style) style).cssPosition = this;
    }
    ...
    public CssProperty getPropertyInStyle(CssStyle style, boolean resolve) {
        if (resolve) {
            return ((Css2Style) style).getPosition();
        } else {
            return ((Css2Style) style).cssPosition;
        }
    }
}
