// $Id: CssTop.java,v 1.1 2012-09-27 14:57:30 ylafon Exp $
...
public class CssTop extends CssProperty {
    ...
    public void addToStyle(ApplContext ac, CssStyle style) {
        if (((Css2Style) style).cssTop != null)
            style.addRedefinitionWarning(ac, this);
        ((Css2Style) style).cssTop = this;
    }
    ...
    public CssProperty getPropertyInStyle(CssStyle style, boolean resolve) {
        if (resolve) {
            return ((Css2Style) style).getTop();
        } else {
            return ((Css2Style) style).cssTop;
        }
    }
}
