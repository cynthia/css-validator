...
public class CssVoiceFamily extends CssProperty {
    ...
    public void addToStyle(ApplContext ac, CssStyle style) {
        if (((Css2Style) style).cssVoiceFamily != null)
            style.addRedefinitionWarning(ac, this);
        ((Css2Style) style).cssVoiceFamily = this;
    }
    ...
    public boolean equals(CssProperty property) {
        return (property instanceof CssVoiceFamily &&
    ...
    public CssProperty getPropertyInStyle(CssStyle style, boolean resolve) {
        if (resolve) {
            return ((Css2Style) style).getVoiceFamily();
        } else {
            return ((Css2Style) style).cssVoiceFamily;
        }
    }
}
